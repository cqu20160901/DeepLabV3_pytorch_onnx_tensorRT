...
class DeepLabv3_plus(nn.Module):
    def __init__(self, nInputChannels=3, num_classes=2, os=16, pretrained=False, freeze_bn=False, _print=False):
        if _print:
...
def test_image(img_path):
    ...
    model = DeepLabv3_plus(nInputChannels=3, num_classes=2, os=16, pretrained=False, _print=True)
    model.eval()
    ...
    img_data = Variable(img_data.unsqueeze(0))

    output = model.forward(img_data)
    output = np.array(output.detach().numpy()).reshape(-1)
    print(output.shape)
    ...
    cv2.imwrite('./test_result_pytorch.jpg', opencv_image)
